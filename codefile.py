...
# Step 3: Ask for Ideal Spending for Top 3 Categories
    ...
    st.write(f"### How much would you *ideally* like to spend per month on the following categories?")
    
    ideal_spending = {}
    for category in st.session_state.top_categories:
        ideal_spending[category] = st.number_input(f"How much would you ideally spend on **{category}** per month?", min_value=0, step=10)
...
# Step 4: Ask for Actual Spending for Top 3 Categories
elif st.session_state.step == "ask_actual_spending":
    st.write(f"### How much do you *actually* spend per month on the following categories?")

    actual_spending = {}
    for category in st.session_state.top_categories:
        actual_spending[category] = st.number_input(f"How much do you actually spend on **{category}** per month?", min_value=0, step=10)
...
# Step 5: Show Spending Comparison
    ...
    st.write("### Spending Comparison")
    total_difference = 0
    for category in st.session_state.top_categories:
        ideal = st.session_state.ideal_spending.get(category, 0)
        actual = st.session_state.actual_spending.get(category, 0)
    ...
    st.write(f"### Total Difference Across Categories: **${total_difference}**")
